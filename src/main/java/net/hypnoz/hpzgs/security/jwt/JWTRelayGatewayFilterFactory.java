...
import static net.hypnoz.hpzgs.security.jwt.JWTFilter.AUTHORIZATION_HEADER;
    ...
    public GatewayFilter apply(Object config) {
        return (exchange, chain) -> {
            ...
            if (StringUtils.hasText(token) && this.tokenProvider.validateToken(token)) {
                ServerHttpRequest request = exchange.getRequest().mutate().header(AUTHORIZATION_HEADER, "Bearer "
                        + token).build();
